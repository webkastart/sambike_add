...
- Meta Pixel requires a valid `NEXT_PUBLIC_META_PIXEL_ID` and a separately persisted `AppSetting.metaPixelEnabled` toggle from `/admin/nastavenia`; even when enabled, the client script loads only after marketing consent.
