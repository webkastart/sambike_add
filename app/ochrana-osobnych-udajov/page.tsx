...
export default async function PrivacyPage() {
  ...
  const operator = settings.privacyOperatorName;
  const address = settings.privacyOperatorAddress;
  const email = settings.privacyContactEmail;
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-14 text-[var(--ink)] sm:px-8 sm:py-20">
      ...
      <p className="mt-3 text-sm text-[#6f6d6d]">Verzia zásad: {settings.privacyPolicyVersion}</p>
      {!configured && (
